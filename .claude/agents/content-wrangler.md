...
- {{PROJECT-SPECIFIC HARD RULES — examples from the extracted project:
  purchases are the owner's alone, never the agent's; voice data only from
  corpora whose contributors donated their voices for speech technology;
  no identifiable public figures, ever. Write yours here; these are the
  rules where one violation is one too many.}}
